...
public final class EditBlockListScreen extends Screen
{
	...
	public void init()
	{
		...
		addDrawableChild(
			removeButton = ButtonWidget.builder(Text.literal("移除选中的"), b -> {
				blockList
		...
		addDrawableChild(ButtonWidget.builder(Text.literal("重置为默认值"),
			b -> client.setScreen(new ConfirmScreen(b2 -> {
				...
				client.setScreen(EditBlockListScreen.this);
			}, Text.literal("重置为默认值"), Text.literal("您确定吗？"))))
		...
		float partialTicks)
	{
		...
		if(blockNameField.getText().isEmpty() && !blockNameField.isFocused())
			context.drawTextWithShadow(client.textRenderer, "方块名称或ID", 68,
				height - 50, Colors.GRAY);
