...
public class ZoomManagerScreen extends Screen implements PressAKeyCallback
{
	...
	public void init()
	{
		...
		addDrawableChild(ButtonWidget
			.builder(Text.literal("更多"), b -> level.increaseValue())
			.dimensions(width / 2 - 79, height / 4 + 72 - 16, 50, 20).build());
		
		addDrawableChild(ButtonWidget
			.builder(Text.literal("更少"), b -> level.decreaseValue())
		...
		addDrawableChild(scrollButton = ButtonWidget
			.builder(Text.literal("使用鼠标滚轮: " + onOrOff(scroll.isChecked())),
				b -> toggleScroll())
		...
		float partialTicks)
	{
		...
		context.drawTextWithShadow(textRenderer,
			"缩放级别: " + level.getValueString(), width / 2 - 75, height / 4 + 44,
			0xcccccc);
