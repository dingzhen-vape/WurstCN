...
public final class FishingSpotManager
{
	...
	public boolean onCast()
	{
		...
		if(nextSpot == null && (nextSpot = chooseNextSpot()) == null)
		{
			...
			ChatUtils
				.message("AutoFish mcMMO模式需要另一个钓鱼点。");
			ChatUtils.message("移动您的摄像机（如果有必要，移动玩家）"
				+ " 使钓饵落在红色方框外，然后"
				+ " 投掷鱼竿。");
			spot2MsgShown = true;
		...
		{
			ChatUtils.message("完成！AutoFish现在将自动运行"
				+ " 并根据需要在钓鱼点之间切换。");
