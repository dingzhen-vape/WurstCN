...
public final class CrystalAuraHack extends Hack implements UpdateListener
{
	private final SliderSetting range = new SliderSetting("范围",
		"确定 CrystalAura 放置和引爆水晶的最大范围。", 6, 1, 6, 0.05, ValueDisplay.DECIMAL);
	
	private final CheckboxSetting autoPlace = new CheckboxSetting("自动放置水晶",
		"启用时，CrystalAura 会自动在有效实体附近放置水晶。\n" + "禁用时，CrystalAura 只会引爆手动放置的水晶。",
		true);
	
	private final FacingSetting faceBlocks = FacingSetting
		.withPacketSpam("面向水晶", "CrystalAura 是否应在放置和左键点击终末水晶时面向正确方向"
			+ "放置和左键点击终末水晶。\n\n" + "速度较慢但有助于防作弊插件。", Facing.OFF);
	...
	private final EnumSetting<TakeItemsFrom> takeItemsFrom =
		new EnumSetting<>("从中获取物品", "在哪里寻找终末水晶。", TakeItemsFrom.values(),
