...
public class WurstServerPinger
{
	...
	public void ping(String ip, int port)
	{
		...
		new Thread(() -> pingInCurrentThread(ip, port),
			"Wurst服务器Ping #" + threadNumber.incrementAndGet()).start();
	}
	
	private void pingInCurrentThread(String ip, int port)
	{
		MultiplayerServerListPinger pinger = new MultiplayerServerListPinger();
		System.out.println("正在Ping " + ip + ":" + port + "...");
		
		try
		{
			pinger.add(server, () -> {}, () -> {});
			System.out.println("Ping成功：" + ip + ":" + port);
			
		}catch(UnknownHostException e)
		{
			System.out.println("未知主机：" + ip + ":" + port);
			failed = true;
			
		}catch(Exception e2)
		{
			System.out.println("Ping失败：" + ip + ":" + port);
			failed = true;
		}
