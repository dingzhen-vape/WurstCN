...
public final class BookOffersSetting extends Setting
{
	...
	public void fromJson(JsonElement json)
	{
		...
		{
			System.out.println("无效的书交易列表：" + json);
			e.printStackTrace();
			resetToDefaults();
		}
	}
	...
	private BookOffer loadOffer(WsonObject wson)
	{
		try
		{
			...
			int level = wson.getInt("等级");
			int price = wson.getInt("最大价格", 64);
			return new BookOffer(id, level, price);
			
		}catch(JsonException e)
		{
			System.out.println("无效的书交易：" + wson);
			e.printStackTrace();
			return null;
		}
	}
	...
	public JsonElement toJson()
	{
		...
		offers.forEach(offer -> {
			...
			jsonOffer.addProperty("等级", offer.level());
			if(offer.price() < 64)
				jsonOffer.addProperty("最大价格", offer.price());
	...
	public JsonObject exportWikiData()
	{
		...
		json.add("默认交易", jsonDefaultOffers);
