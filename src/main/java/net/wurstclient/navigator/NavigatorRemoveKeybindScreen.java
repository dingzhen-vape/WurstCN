...
public class NavigatorRemoveKeybindScreen extends NavigatorScreen
{
	...
	private String text = "选择您要移除的快捷键。";
	...
	{
		removeButton = ButtonWidget.builder(Text.literal("移除"), b -> remove())
			.dimensions(width / 2 - 151, height - 65, 149, 18).build();
		removeButton.active = !selectedKey.isEmpty();
		addDrawableChild(removeButton);
		
		addDrawableChild(ButtonWidget
			.builder(Text.literal("取消"), b -> client.setScreen(parent))
		...
		float partialTicks)
	{
		...
		// title bar
		context.drawCenteredTextWithShadow(tr, "移除快捷键", middleX, 32, txtColor);
